...
-(id)initWithTitles:(NSArray *)titles{
    ...
    return [self initWithTitles:titles withButtonHeight:40.0f withMenuColor:[UIColor lightGrayColor] withBackBlurStyle:UIBlurEffectStyleDark];
}
...
- (void)configAvatorImageView {
    UIImageView *imageview = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"navigation_LeftImage"]];
    imageview.frame = CGRectMake(keyWindow.width/4 - 40, keyWindow.height / 2 - 125 - 50 - 80 , 80, 80);
         ...
         usingSpringWithDamping:0.5
          initialSpringVelocity:0
                     ...
                     animations:^{
                         ...
                         self.curveView.frame = CGRectMake(0, SYS_DEVICE_HEIGHT/2, 3, 3);
                     } completion:^(BOOL finished) {
                         
                         if(finished)
                         {
                             self.isAnimating = NO;
                             self.isOpeningMenu = NO;
                             [self endAnimation];
                         }
                     }];
}
...
-(void)addButtons:(NSArray *)titles{
    ...
    if (titles.count % 2 == 0) {
        ...
        for (NSInteger i = 0; i < titles.count; i++) {
            ...
            LYMenuButton *home_button = [[LYMenuButton alloc]initWithTitle:title];
            if (i >= titles.count / 2) {
